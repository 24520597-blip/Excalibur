"""Main TUI application for Excalibur using Textual framework."""
...
from excalibur.interface.components.activity_feed import ActivityFeed
from excalibur.interface.components.splash import SplashScreen
...
class HelpScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Grid(
            Label("🚩 Excalibur CTF Solver - Help", id="help_title"),
            Label(
                "F1        Help\n"
...
class QuitScreen(ModalScreen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Grid(
            Label("🚩 Quit Excalibur CTF Solver?", id="quit_title"),
            Grid(
...
class ExcaliburApp(App[None]):
    """Main TUI application for Excalibur CTF Solver."""

    CSS_PATH = "styles.tcss"
    TITLE = "Excalibur - CTF Challenge Solver"
    ...
    def _on_tree_event(self, event: Event) -> None:
        """Handle EGATS tree events for activity feed."""
        if not self._activity_feed:
            return
        etype = event.type
        data = event.data
        msg = ""
        if etype == EventType.TREE_NODE_SELECTED:
            msg = f"[EGATS] Selected node: {data.get('node_id', '')[:8]}"
        elif etype == EventType.TDI_COMPUTED:
            msg = f"[EGATS] TDI={data.get('tdi_value', 0):.2f}"
        elif etype == EventType.MODE_SELECTED:
            msg = f"[EGATS] Mode: {data.get('mode', 'unknown')}"
        elif etype == EventType.TREE_NODE_PRUNED:
            pruned = data.get("pruned_ids", [])
            msg = f"[EGATS] Pruned {len(pruned)} node(s)"
        elif etype == EventType.TREE_PIVOT_SPAWNED:
            msg = f"[EGATS] Pivot spawned: {data.get('host', '')}"
        if msg:
    ...
    def _create_header(self) -> Static:
        ...
        header_text.append("Excalibur", style="bold #6366f1")
    ...
    def _build_status_text(self) -> Text:
        ...
        if self._debug:
            from excalibur.core.agent import DEBUG_LOG
    ...
    def _start_agent(self) -> None:
        ...
        from excalibur.core.config import load_config
