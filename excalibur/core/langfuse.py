"""Langfuse observability integration for Excalibur.
...
def _get_or_create_user_id() -> str:
    ...
    The user ID is stored in ~/.excalibur/user_id and persists across sessions.
    This allows tracking usage patterns per user in Langfuse.

    Returns:
        A UUID string identifying this user.
    """
    user_id_file = Path.home() / ".excalibur" / "user_id"
...
def init_langfuse(disabled: bool = False) -> bool:
    ...
    Telemetry is enabled by default to help improve Excalibur.
    ...
    # Hardcoded telemetry configuration for Excalibur project
            ...
            if _pending_session and not _current_span:
                with contextlib.suppress(Exception):
                    _current_span = _langfuse_client.start_span(
                        name=f"excalibur:{_pending_session['target']}",
                        input={
