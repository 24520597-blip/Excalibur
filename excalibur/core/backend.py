...
_active_gemini_backend: ContextVar[Any | None] = ContextVar(
    "active_gemini_backend",
    default=None,
)


def _gemini_shell_tool(command: str, timeout: int = 300) -> dict[str, Any]:
    """Execute a shell command through the active Gemini backend.

    This must remain a module-level function. The Gemini SDK deep-copies
    callable tools before each request, and bound methods retain client
    objects containing thread locks that cannot be copied.
    ...
    """
    backend = _active_gemini_backend.get()
    if backend is None:
        return {
            "exit_code": -1,
            "stdout": "",
            "stderr": "No active Gemini backend is available",
        }
    return cast(dict[str, Any], backend._run_shell_command(command, timeout))
...
class GeminiBackend(AgentBackend):
    ...
    async def query(self, prompt: str) -> None:
        ...
        context_token = _active_gemini_backend.set(self)
        try:
            ...
            _active_gemini_backend.reset(context_token)
